...
def run_multiple():
    ...
    for t in task_list:
        fns = glob.glob(f"{data_dir}*.pkl")
        for i, fn in enumerate(fns):
            #fn = data_dir + t + str(j) + '.pkl'
            print('Analyzing ', fn)
            analyze_model_from_file(fn, savefile = f"{i}_analysis.pkl", update_params = update_params)
            print(0/0)
...
def analyze_model_from_file(filename, savefile = None, update_params = {}):
    ...
    for key, val in results['weights'].items():
        if type(val) == np.ndarray:
            print(key, val.shape)
            results['weights'][key] = val.squeeze()
    ...
    '''for r in np.unique(trial_info['rule']):
        ...
        #print(ind, trial_info['desired_output'].shape, y.shape, trial_info['train_mask'].shape)
    ...
    if par['simulation_reps'] > 0:
        ...
        simulation_results = inactivation_experiments(trial_info, h, syn_x, syn_u, results['weights'], num_reps=10)
    
    # Save the analysis results    
    pickle.dump(simulation_results, open(savefile, 'wb'))
    print(f'Analysis results saved in {savefile}')
...
def average_test_response(h, trial_info, test_onset):
    ...
    h_test = np.mean(h[test_onset:, :, : ], axis=0).squeeze()
    print(h_test.shape)
...
def inactivation_experiments(trial_info, h, syn_x, syn_u, network_weights, num_reps = 5):
    ...
    elif  par['trial_type'] in ['DMS', 'DMC', 'DMRS90', 'DMRS90ccw', 'DMS+DMRS+DMC']:
        test_onset = []
    ...
    # For storing accuracies + activities
    simulation_results = {}

    # For masking correct parts of trial 
    ...
    silencing_fractions = np.arange(0., 1., 0.25)

    # Now: extinguish fraction of the connections from PFC module to LIP module and simulate
    for r, t in product(range(2,3), range(num_test_periods)):
        print(f"Rule: {r}, t: {t}")

        # Some setup (to make shapes work)
        ...
        pfc_to_lip_conns = np.arange(40, 160)
        lip_ids = np.arange(160, 360)

        for f in silencing_fractions:
            print(f"Silencing {f*100}% of PFC-to-LIP connections.")

            for n in range(num_reps):

                # Connections: select which PFC-to-LIP connections should be silenced
                projs_to_silence = np.random.choice(pfc_to_lip_conns, 
                                                    size=int(f*len(pfc_to_lip_conns)), 
                                                    replace=False)
                network_weights_copy = copy.copy(network_weights)
                network_weights_copy['w_rnn'][projs_to_silence, 160:360] = 0.
                ...
                y, h_comp, _, _  = run_model(x, hidden_init, syn_x_init, syn_u_init, network_weights_copy)

                # Store results
                results_bundle = {'accuracy': get_perf(desired_output, y, train_mask)[0], 
                                  'activities': h_comp}
                #print(h_comp.shape)
                #print(0/0)
                results_key = (r, t, f, n)
                simulation_results[results_key] = results_bundle


                # Units: select which PFC units should be silenced
                '''hidden_init = np.copy(h[test_onset[t]-1,trial_ind,:])
                syn_x_init = np.copy(syn_x[test_onset[t]-1,trial_ind,:])
                syn_u_init = np.copy(syn_u[test_onset[t]-1,trial_ind,:])
    ...
    # Also save out trial info 
    simulation_results['trial_info'] = {'rule': trial_info['rule'], 
                                        'sample_dir': trial_info['sample'], 
                                        'test_dir': trial_info['test'],
...
def calculate_tuning(h, syn_x, syn_u, trial_info, trial_time, network_weights, calculate_test = False):
    ...
    if par['trial_type'] == 'dualDMS':
        ...
        par['batch_train_size'] = trial_info['sample'].shape[0]
        rule = np.array(trial_info['rule'])
        match = np.array(trial_info['match'])
        sample = np.reshape(np.array(trial_info['sample']),(par['batch_train_size'], 1))
        test = np.reshape(np.array(trial_info['test']),(par['batch_train_size'], 1))
...
def run_model(x, h_init_org, syn_x_init_org, syn_u_init_org, weights, suppress_activity = None):
    ...
    return y.squeeze(), h.squeeze(), syn_x.squeeze(), syn_u.squeeze()
...
def get_perf(target, output, mask, to_print=False):
...
def get_perf_sr(target, output, mask, to_print=False):
    ...
    if to_print:
        for i in range(target.shape[0]):
            print(i, mask_test[i,0], target_max[i,0], output_max[i,0])
        print(np.sum(np.float32(target_max[:,0] == output_max[:,0])*mask_test[:,0]))
    #print(0/0)
    accuracy = np.sum(np.float32(target_max == output_max)*mask_test)/np.sum(mask_test)
    return accuracy


def get_perf_single_output(target, output, mask, to_print=False):
    ...
    for i in range(mask.shape[1]):
        z_ind = np.where(mask[:,i].flatten() == 0)[0][0]
        test_time[z_ind:, i] = 3 
    #test_time[np.where(mask == 0)[0]:, :] = 3
    mask_test = mask_full * np.float32(test_time == 3)
    #mask_test = mask_full*(target[:,:,0]==0)
    mask_test = mask_full*test_time

    ## CHANGE THIS FOR DMC VS DNMC
    mask_non_match = mask_full*(target[:,:,1]==0)
    mask_match = mask_full*(target[:,:,1]==1)
    target_max = np.argmax(target, axis = 2)
    output_max = np.argmax(output, axis = 2)
    #processed_output = np.heaviside(output[:,:,1], 0)
    #accuracy = np.sum(np.float32(target[:,:,1].squeeze() == processed_output)*mask_test)/np.sum(mask_test)
    accuracy = np.sum(np.float32(target_max == output_max)*mask_test)/np.sum(mask_test)
