...
def exc_diff_timedep_func(V_rest, V_th, tau_ref, gain, V_m, spikes, input, exc):
    ...
    exc_rest = 10 * sigmoid(num_spikes - spike_offset) + V_rest - (exc[0, -1] - V_rest) * dt / tau_exc + np.random.normal(0, 0.5)
    # V_rest <= exc_rest <= exc_rest_max
    exc_rest = min(max(exc_rest, V_rest), par['exc_rest_max'])

    exc_th = -5 * (np.tanh(num_spikes - spike_offset) + 1)/2 + V_th - (exc[1, -1] - V_th) * dt / tau_exc + np.random.normal(0, 0.25)
    # exc_thresh_min <= exc_th <= V_th
    exc_th = min(max(exc_th, par['exc_thresh_min']), V_th)

    exc_refrac = -0.002 * (math.erf(num_spikes - spike_offset) + 1)/2 + tau_ref - (exc[2, -1] - tau_ref) * dt / tau_exc + np.random.normal(0, 0.000125)
    # tau_abs_ref <= exc_refrac <= tau_ref
    exc_refrac = min(max(exc_refrac, par['tau_abs_ref']), tau_ref)
