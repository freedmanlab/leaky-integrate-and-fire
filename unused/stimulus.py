...
class Stimulus:
    ...
    def generate_trial(self, test_mode = False, set_rule = None, memory_bank_id=None):
        ...
        if par['trial_type'] in ['DMS','DMRS45','DMRS90','DMRS90ccw','DMRS180','DMC',\
            ...
            trial_info = self.generate_sequence_reproduction_trial(test_mode, memory_bank_id)
    ...
    '''
    def make_memory_banks():
        """ Make all memory banks, + save pattern indices in dict."""

        # 1. Compute number of possible patterns
        if par['allow_repeated_symbols']:
            n_possible_sequences = scipy.misc.comb(par['symbol_set_length'] + par['pattern_length'] - 1, 
                                                   par['pattern_length'])
        else:
            n_possible_sequences = scipy.misc.comb(par['symbol_set_length'], par['pattern_length'])

        for bank in range(par['n_memory_banks']):

             # Require unique starts (up to seed_length)
            symbol_set = np.arange(1, par['symbol_set_length'] + 1)
            all_seeds = list(itertools.permutations(symbol_set, 
                                                    par['seed_length']))
            reproduction_shape = (len(all_seeds),
                                  par['pattern_length'] - par['seed_length'])


            for pattern in range(par['n_patterns']):




    def generate_pattern_by_id(id):
        """ Given the ID of a pattern, return the pattern. """

        return
    '''


    def generate_sequence_reproduction_trial(self, test_mode, memory_bank_id=None):
        """
        Generate a sequence reproduction trial.

        Trial outline
        1. Dead period
        2. Sequence of stimuli presented
        4. Delay (NULL/fixation input)
        5. First k stimuli of sequence presented 
        6. GO cue
        7. Test period (remaining n - k tokens need to be produced)

        TODO:
        - remove hard-coding of patterns + related magic numbers
        - make more systematic re fixation and go -- now, fix is 0, go is last
        ...
        # Only do this if the pattern set for the current memory bank not
        # yet created
        if memory_bank_id is None:
            memory_bank_id = par['memory_bank_id']
        if memory_bank_id in par['memory_banks'].keys():
            pattern_set = par['memory_banks'][memory_bank_id]
            ...
            if par['allow_repeated_symbols']:
                pattern_ends = np.random.choice(symbol_set, 
                                                size=reproduction_shape)
            else:
                pattern_ends = np.array([np.random.choice(symbol_set, 
                                                size=par['pattern_length'] - par['seed_length'], 
                                                replace=False) for i in range(len(all_seeds))])

            # Winnow down set of uniquely-seeded patterns to par['n_patterns']
            inc_inds = np.random.choice(len(all_seeds), 
                                        par['n_patterns'], 
                                        replace=False)
            pattern_set = np.array([list(all_seeds[i]) + list(pattern_ends[i]) for i in inc_inds])

            # Add to dict
            par['memory_banks'][memory_bank_id] = pattern_set

        # Including now to be able later to test whether we need
        # to explicitly show the network the patterns that we later want it to reproduce
        null_pattern = np.array([0, 0, 0, 0, 0])
        ...
        trial_info = {'desired_output'  :  np.zeros((par['num_time_steps'], par['batch_size'], par['n_output']),dtype=np.float32),
                      ...
                      'pattern'         :  np.zeros((par['batch_size'], par['pattern_length']), dtype=np.float32),
                      'neural_input'    :  np.random.normal(par['input_mean'], par['noise_in'], size=(par['num_time_steps'], par['batch_size'], par['n_input']))}

        # Create indices for time periods
        if par['show_patterns']:
            dead_time_rng  = range(par['dead_time'] // par['dt'])
            pres_time_rng  = range(par['dead_time'] // par['dt'], 
                                   par['dead_time'] // par['dt'] + (par['pattern_length'] * par['symbol_time']) // par['dt'])
            delay_time_rng = range(pres_time_rng[-1], 
                                   pres_time_rng[-1] + par['delay_time'] // par['dt'])
            seed_time_rng  = range(delay_time_rng[-1], 
                                   delay_time_rng[-1] + (par['seed_length'] * par['symbol_time']) // par['dt'])
            repr_time_rng  = range(seed_time_rng[-1],
                                   seed_time_rng[-1] + ((par['pattern_length'] - par['seed_length']) * par['symbol_time']) // par['dt'])

        else:
            dead_time_rng  = range(par['dead_time'] // par['dt'])
            seed_time_rng  = range(par['dead_time'] // par['dt'], 
                                   par['dead_time'] // par['dt'] + (par['seed_length'] * par['symbol_time']) // par['dt'])
            repr_time_rng  = range(seed_time_rng[-1],
                                   seed_time_rng[-1] + ((par['pattern_length'] - par['seed_length']) * par['symbol_time']) // par['dt'])

        # Set mask to 0 during dead period
        trial_info['train_mask'][:repr_time_rng[0], :] = 0

        ## INPUTS
        for t in range(par['batch_size']):

            # Choose presented pattern, if presenting a pattern first
            if par['show_patterns']:
                pres_pattern_id = np.random.choice(pattern_set.shape[0])
                pres_pattern = pattern_set[pres_pattern_id, :]

            # Choose test pattern
            test_pattern_id = np.random.choice(pattern_set.shape[0])
            test_pattern = pattern_set[test_pattern_id, :]
            seed_pattern = test_pattern[:par['seed_length']]
            repr_pattern = test_pattern[par['seed_length']:]

            # INPUT
            if par['show_patterns']:
                one_hot_inputs_pres = np.zeros((len(pres_pattern), par['n_input']))
                one_hot_inputs_pres[np.arange(len(pres_pattern)), pres_pattern] = 1.

            one_hot_inputs_seed = np.zeros((len(seed_pattern), par['n_input']))
            one_hot_inputs_seed[np.arange(len(seed_pattern)), seed_pattern] = 1.

            # Stretch inputs into correct number of timesteps per one-hot
            if par['show_patterns']:
                pres_inputs = np.repeat(one_hot_inputs_pres, par['symbol_time'] // par['dt'], 0)
            seed_inputs = np.repeat(one_hot_inputs_seed, par['symbol_time'] // par['dt'], 0)

            if par['show_patterns']:
                trial_info['neural_input'][pres_time_rng,  t, :]  += par['tuning_height'] * pres_inputs
                trial_info['neural_input'][delay_time_rng, t, 0]  += par['tuning_height'] 
            trial_info['neural_input'][seed_time_rng,  t, :]  += par['tuning_height'] * seed_inputs
            trial_info['neural_input'][repr_time_rng,  t, -1] += par['tuning_height']

            # DESIRED OUTPUT
            one_hot_outputs_repr = np.zeros((len(repr_pattern), par['n_output']))
            one_hot_outputs_repr[np.arange(len(repr_pattern)), repr_pattern] = 1.
            repr_outputs = np.repeat(one_hot_outputs_repr, par['symbol_time'] // par['dt'], 0)
            if par['show_patterns']:
                trial_info['desired_output'][pres_time_rng,  t, 0] = 1.
                trial_info['desired_output'][delay_time_rng, t, 0] = 1.
            trial_info['desired_output'][seed_time_rng,  t, 0] = 1.
            trial_info['desired_output'][repr_time_rng,  t, :] = repr_outputs

            # For ease of use in model w/ LSTM, save last timepoint of seed
            par['last_seed_timept'] = seed_time_rng[-1]

            # Store trial info for saving
            trial_info['pattern'][t, :] = test_pattern
    ...
    def generate_basic_trial(self, test_mode, set_rule = None):
        ...
        for t in range(par['batch_size']):
            ...
            if par['num_fix_tuned'] > 0:
                trial_info['neural_input'][fix_time_rng, t] += np.reshape(self.fix_tuning[:,0],(-1,1)).T
            ...
            if not catch:
                ...
                if match == 0:
                    if not par['in_development']:
                        trial_info['desired_output'][test_time_rng, t, 1] = 1. ## NON-MATCH unit
                    else:
                        trial_info['desired_output'][test_time_rng, t, 1] = 1. ## for 2 output case: fixation = non-match
                else:
                    if not par['in_development']:
                        trial_info['desired_output'][test_time_rng, t, 2] = 1. ## MATCH unit
                    else:
                        trial_info['desired_output'][test_time_rng, t, 0] = 1. ## for 2 output case: second output = match
            else:
