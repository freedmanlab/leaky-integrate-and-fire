...
def spikes_to_spikepulse(time, tau):
    return .015*(np.exp(-(time/(4*tau)))-np.exp(-(time/tau)))
...
par = {
    ...
    'simulation_dt'         : 0.2,   # Simulation timestep (ms)
    'V_in'                  : .055,      # Neuron input voltage
    'input_duration'        : 50.0,     # duration of the input current (ms)

    'gain'                  : 1.0,      # neuron gain (unitless)
    't_rest'                : 0.05,        # initial refractory time (ms? what is this lol)
    'Rm'                    : 10.0,        # Resistance (MOhm)
    'Cm'                    : 1,       # Capacitance (nF)
    'tau_exc'               : 100,      # time constant for decay of exc properties (ms), >> tau_m
    ...
    'input_stdev'           : .005,     #standard deviation of the input Gaussian noise
    'voltage_stdev'         : .0015,     #standard deviation of the neuron voltage update, also Gaussian
    ...
    'num_neurons'           : 100,
    'num_inputs'            : 3,
    'exc_prop'              : .9,     #proportion of neurons that are excitatory
